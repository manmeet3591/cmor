...
dformat3 = 'atm_6hr_%Y_%m_%d_%H_plev.nc'
...
vardf = pd.read_csv('A6hourly_4lev_varlist',skipinitialspace=True)
...
iplev4 = cmor.axis(table_entry= 'plev4',
         units= 'Pa',
	 coord_vals=plevs)
...
while ddate < enddate:
	...
	mdatec=sp.check_output(['./datefunc', 'incdatemid', 'julian '+ddatec+' 0,0,0,6,0,0'])
	...
	#	itime1 = cmor.axis(table_entry= 'time1',
		...
		axis_ids = [itime,iplev4,ilat,ilon]
